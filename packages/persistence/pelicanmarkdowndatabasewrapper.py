...
class PelicanMarkdownDatabaseWrapper(AbstractDatabaseWrapper):
    ...
    def setup(self):
        tblstmts = ["CREATE TABLE IF NOT EXISTS post (id INTEGER PRIMARY KEY NOT NULL, title TEXT NOT NULL, author TEXT NOT NULL, content TEXT, status TEXT DEFAULT 'draft' CHECK (status == 'draft' or status == 'published'), tmsp_create NUMERIC DEFAULT CURRENT_DATETIME, tmsp_publish NUMERIC)"
                    , "CREATE TABLE IF NOT EXISTS user (id INTEGER PRIMARY KEY NOT NULL, authorized INTEGER DEFAULT 0)"]
